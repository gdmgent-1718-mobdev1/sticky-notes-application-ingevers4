...
var ApplicationDbContext = {
    "init": function(connectionStr) {
        ...
        // Get the stored data with the key. If the data is not present in the localstorage
        // --> store the previous data from the variable _dbData into the localstorage via the connection string or namespace
        //als er data bestaat, pars ze dan naar een json string naar de local storage
        if(window.localStorage.getItem(this._connectionStr) != null) {
            //JSON.parse zet de code in een string om naar bruikbare javascript code
    ...
    "deleteStickyNoteById": function(id) {
        ...
        //de index kan niet -1 zijn, moet sowieso positief zijn, dus zo kan je dit testen. 
        if(index == -1) {
            return false;
        }
